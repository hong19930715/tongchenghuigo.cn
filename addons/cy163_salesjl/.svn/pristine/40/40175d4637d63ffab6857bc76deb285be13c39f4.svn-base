...
if($operation == 'display'){
	...
	$psize = 10;
	$total = pdo_fetchcolumn("SELECT COUNT(*) FROM ".tablename(BEST_ORDER)." WHERE weid = {$_W['uniacid']} AND status = -2 AND refund_status = 0");
	$list = pdo_fetchall("SELECT * FROM ".tablename(BEST_ORDER)." WHERE weid = {$_W['uniacid']} AND status = -2 AND refund_status = 0 ORDER BY tktime DESC LIMIT ".($pindex - 1)*$psize.",".$psize);	
	foreach($list as $k=>$v){
		$memberrz = pdo_fetch("SELECT rzrealname FROM ".tablename(BEST_MEMBERRZ)." WHERE openid = '{$v['openid']}' AND rzstatus = 1 AND isjujue = 0");
		if(empty($memberrz)){
			$list[$k]['realname'] = "未实名";
		}else{
	...
	if($tuikuan['refund_price'] < 0 || $tuikuan['refund_status'] != 0){
		returnError('该记录不能被提现！');
	}
	try {
		$ret=$this->refundByPay(array(
			'transaction_id'=>$tuikuan['transid'],
			'total_fee'=>$tuikuan['refund_price'],
