...
//$datahd['shname'] = $_GPC['shname'];
//$datahd['shphone'] = $_GPC['shphone'];
...
if (!empty($_GPC['shname'])&&!empty($_GPC['shphone'])){
    $update = array(
        'shname' => $_GPC['shname'],
        'shphone' => $_GPC['shphone']
    );
    pdo_update(BEST_MEMBER, $update, array('openid' => $openid));
}
