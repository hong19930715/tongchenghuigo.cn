...
if(!empty($datahd['shname'])){
    $datahd['nickname'] = trim($_GPC['shname']);
}else{
...
if(empty($datahd['title'])){
	$this->result(1,"请填写接龙主题名称！", $datahd);
}
$datahd['des'] = $_GPC['des'];
if(empty($datahd['des'])){
...
pdo_update(BEST_MEMBERHUODONG,$datahd,array('id'=>$jlid));
//$hdid = pdo_insertid();
//$hdid = $jlid;
//pdo_delete(BEST_MEMBERGOODS,array('hdid'=>$jlid));
foreach($goods as $k=>$v){
	...
	$gthumbs = array();
	foreach($v['thumbs'] as $kk=>$vv){
		$gthumbs[] = $vv['realpath'];
	}
	$data['thumbs'] = serialize($gthumbs);
	if ($v['hdid']==$jlid){
        pdo_update(BEST_MEMBERGOODS,$data,array('hdid'=>$jlid));
    }else{
        $data['hdid'] = $jlid;
        pdo_insert(BEST_MEMBERGOODS,$data);
        $jlid = pdo_insertid();
    }
//	$goodsid = pdo_insertid(); if(pdo_exists(BEST_MEMBERGOODSJIETI,array('goodsid'=>$jlid))){
    pdo_delete(BEST_MEMBERGOODSJIETI,array('goodsid'=>$jlid));
	foreach($v['jietilist'] as $kkk=>$vvv){
		$datajt['goodsid'] = $jlid;
...
//		pdo_insert(BEST_MEMBERGOODSJIETI,$datajt,array('goodsid'=>$jlid));
	}
}
$this->result(0,"更新成功！等待管理员审核", '');
?>